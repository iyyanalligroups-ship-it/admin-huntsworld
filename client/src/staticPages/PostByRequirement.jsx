...
import { Clock, ListChecks, ShoppingBag, X, ClipboardList } from "lucide-react";
import {
...
const PostRequirement = () => {
  ...
  useEffect(() => {
    if (user?.user?.role?.role === "GROCERY_SELLER") {
      // navigate("/grocerySeller/requirement");
    }
  }, [user, navigate]);
  ...
  useEffect(() => {
    ...
    if (!user?.user?._id) {
      navigate("/login");
      showToast("Please log in to submit a requirement.",'info');
    }
    if (!initialPhone || initialPhone.trim() === "") {
      showToast("Please update your phone number in your profile settings before submitting a requirement.",'info');
    }
  ...
  if (user?.user?.role?.role === "GROCERY_SELLER") {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center px-4">
        ...
        <Button
          onClick={() => navigate("/grocerySeller/requirement")}
  ...
  return (
    <div>
    {/* {
      user?.user?.role?.role != "SUB_ADMIN" || user?.user?.role?.role != "ADMIN"   &&
            ...
            <CardContent className="p-6">
              <form className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full bg-[#0c1f4d] hover:shadow-lg text-white py-2 rounded-md"
                >
