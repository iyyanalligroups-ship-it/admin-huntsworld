...
export const Authapi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getRoleUserData: builder.query({
      query: ({ page = 1, limit = 10, name = "", role = "USER" }) =>
        `/users/fetch-all-user-role-data?page=${page}&limit=${limit}&name=${name}&role=${role}`,
      providesTags: ["Users"],
    }),
    ...
    deleteUser: builder.mutation({
      ...
      invalidatesTags: ["Users"], // ✅ Ensure UI updates after deleting a user
    }),
    changeToUser: builder.mutation({
      query: (id) => ({
        url: `/users/change-to-user/${id}`,
        method: "PATCH",
      }),
      invalidatesTags: ["Users"], // ✅ Ensure UI updates after deleting a user
    }),
    ...
    addUserAddress: builder.mutation({
      ...
      invalidatesTags: ["User", "Users"], // ✅ Ensure UI updates after adding an address
    }),
    ...
    updateUserVerification: builder.mutation({
      query: ({ userId, email_verified }) => ({
        url: `/users/update-verification/${userId}`,
        method: "PATCH",
        body: { email_verified },
      }),
      invalidatesTags: ["User", "Users"], // Invalidate both tags to ensure all user-related queries refresh
    }),
