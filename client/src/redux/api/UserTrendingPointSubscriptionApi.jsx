...
export const TrendingPointsPaymentApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getUserProducts: builder.query({
      query: (userId) => `/products/fetch-all-products-for-seller/${userId}`,
    ...
    grantFreeTrendingPoints: builder.mutation({
    query: ({ user_id, subscription_id, points, reason }) => ({
      url: '/trending-points-payment/free-grant',   // ← must match your backend route
      method: 'POST',
      body: { user_id, subscription_id, points, reason: reason || undefined },
    }),
    invalidatesTags: [
      'TrendingPointsPayments',
      'ActiveTrendingPointsUsers',
      'UserSubscription',     // if you want subscription status to refresh too
    ],
  }),
