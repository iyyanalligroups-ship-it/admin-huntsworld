...
export const TrustSealRequestApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getMerchantTrustSealDetails: builder.query({
      query: ({ userId }) =>
        `/trust-seal/fetch-trust-seal-certificate-detail/${userId}`,
      providesTags: ["TrustSealRequests"],
    }),
       deleteTrustSealRequest: builder.mutation({
      query: (requestId) => ({
        url: `/trust-seal/delete-request/${requestId}`,
        method: "DELETE",
      }),
      invalidatesTags: ["TrustSealRequests"],
    }),
    ...
    uploadImagesToImageServer: builder.mutation({
      query: ({ companyName, images }) => {
        ...
        return {
          ...
          }/trust-seal-images/upload-images`,
          method: "POST",
          body: formData,
        };
      },
    ...
    assignTrustSealRequest: builder.mutation({
      query: ({ request_id, student_id, assigned_by }) => ({
        url: '/trust-seal-assignment/assign',
        method: 'POST',
        body: { request_id, student_id, assigned_by },
      }),
      invalidatesTags: ['TrustSealRequests'],
    }),
    reassignTrustSealRequest: builder.mutation({
      query: ({ request_id, student_id, assigned_by }) => ({
        url: '/trust-seal-assignment/reassign',
        method: 'POST',
        body: { request_id, student_id, assigned_by },
      }),
      invalidatesTags: ['TrustSealRequests'],
    }),
    unassignTrustSealRequest: builder.mutation({
      query: (request_id) => ({
        url: `/trust-seal-assignment/unassign/${request_id}`,
        method: 'DELETE',
      }),
      invalidatesTags: ['TrustSealRequests'],
    }),
    getTrustSealRequestDetails: builder.query({
      query: (requestId) => `/trust-seal/notifications/trust-seal/${requestId}`,
      providesTags: ["TrustSealRequests"],
    }),
  }),
...
export const {
  ...
  useDeleteTrustSealRequestMutation,
} = TrustSealRequestApi;
