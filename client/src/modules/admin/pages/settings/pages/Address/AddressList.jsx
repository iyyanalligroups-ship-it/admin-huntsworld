...
const AddressList = () => {
  ...
  const handleSelect = (address) => {
    ...
    if (countryObj?.isoCode && stateObj?.isoCode) {
      const baseCities = City.getCitiesOfState(
        countryObj.isoCode,
        stateObj.isoCode
      ) || [];
  ...
  const handleSubmit = async () => {
    try {
      ...
      const submitData = {
        ...
        city: formData.city || "", // Keep as is (TitleCase)
        user_id: user?.user?._id,
      ...
      if (selectedAddressId) {
        await updateAddress({
          user_id: user?.user?._id,
          selectedAddressId: selectedAddressId, // Use addressId to match API
        ...
        <ScrollArea className="h-[calc(100vh-32px)] pr-4">
          ...
          <div className="relative space-y-4">
            {addresses?.map((address) => (
              <Card
                key={address._id}
                className={`p-4 flex items-start justify-between cursor-pointer transition-all border-2 ${selectedAddressId === address._id
