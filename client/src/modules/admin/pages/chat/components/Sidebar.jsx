...
export default function Sidebar({ onSelect }) {
  ...
  useEffect(() => {
    ...
    setAllUsers((prev) => {
      ...
      const merged = data.users.map((apiUser) => {
      ...
      const extraLocal = prev.filter((u) => !apiIds.has(u._id));
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (msg) => {
      ...
      setAllUsers((prev) => {
        const exists = prev.some((u) => String(u._id) === String(contactId));
  ...
  return (
    <div className="w-full md:w-80 lg:w-96 bg-white flex flex-col h-full">
