...
const UserForm = ({ user, closeModal, refetch, isAdminMode = false }) => {
  const [formData, setFormData] = useState({
    ...
    phoneOtp: "",
    emailOtp: "",
    role: user?.role?.role || (isAdminMode ? "ADMIN" : "USER"),
  });

  const [registerType, setRegisterType] = useState(user?.email || isAdminMode ? "email" : "phone");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="max-h-[60vh] overflow-y-auto px-1 scrollbar-thin scrollbar-thumb-gray-200">
        ...
        {!user && (
          <div className="space-y-2 md:col-span-2">
            ...
            <Select 
              ...
              disabled={phoneOtpShow || (!isAdminMode && phoneVerified)}
            >
              <SelectTrigger className="h-12 rounded-xl bg-white border-2 border-slate-300 shadow-sm focus:border-[#0c1f4d] focus:ring-2 focus:ring-[#0c1f4d]/10 font-bold">
        ...
        <div className="space-y-2">
          ...
          <Input 
            ...
            className="h-12 rounded-xl bg-white border-2 border-slate-300 shadow-sm focus:border-[#0c1f4d] focus:ring-2 focus:ring-[#0c1f4d]/10 font-semibold placeholder:text-gray-300 transition-all" 
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input 
                ...
                maxLength={10} 
                className={`h-12 pl-16 rounded-xl bg-white border-2 border-slate-300 shadow-sm focus:border-[#0c1f4d] focus:ring-2 focus:ring-[#0c1f4d]/10 font-black tracking-[0.2em] placeholder:text-gray-200 transition-all ${phoneVerified ? 'bg-green-50 border-green-300' : ''}`} 
              />
            </div>
            {!phoneVerified && (
          ...
          {phoneOtpShow && !phoneVerified && (
            <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2 mt-2">
              <Input name="phoneOtp" value={formData.phoneOtp} onChange={handleChange} placeholder="e.g. 123456" className="h-11 flex-1 rounded-xl border-2 border-slate-300 bg-red-50/30 text-center font-black tracking-[0.5em] focus:ring-red-500/20" />
          ...
          <div className={`space-y-2 md:col-span-2 transition-opacity duration-300 ${!phoneVerified ? 'opacity-50' : 'opacity-100'}`}>
            ...
            <div className="flex gap-2">
              <Input 
                ...
                placeholder={phoneVerified ? "e.g. john@example.com" : "Verify phone to unlock"} 
                className={`h-12 flex-1 rounded-xl bg-white border-2 border-slate-300 shadow-sm focus:border-[#0c1f4d] focus:ring-2 focus:ring-[#0c1f4d]/10 font-semibold placeholder:text-gray-300 transition-all ${emailVerified ? 'bg-green-50 border-green-300' : ''}`} 
              />
            ...
            {emailOtpShow && !emailVerified && (
              <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2 mt-2">
                <Input name="emailOtp" value={formData.emailOtp} onChange={handleChange} placeholder="e.g. 123456" className="h-11 flex-1 rounded-xl border-2 border-slate-300 bg-red-50/30 text-center font-black tracking-[0.5em] focus:ring-red-500/20" />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input 
                  ...
                  onChange={handleChange} 
                  placeholder="e.g. ••••••••"
                  className="h-12 rounded-xl bg-white border-2 border-slate-300 shadow-sm focus:border-[#0c1f4d] focus:ring-2 focus:ring-[#0c1f4d]/10 font-semibold transition-all" 
                />
      ...
      <div className="flex gap-4 pt-6 border-t border-gray-100">
         ...
         <Button 
          type="submit" 
          disabled={!phoneVerified || isRegistering || isUpdatingUser || isCompleting || !isFormValid()}
          className={`flex-[2] h-14 rounded-xl font-black shadow-xl uppercase text-xs tracking-[0.2em] transition-all cursor-pointer ${phoneVerified && isFormValid() ? 'bg-[#0c1f4d] text-white hover:bg-black' : 'bg-gray-200 text-gray-400'}`}
         >
