...
const Stepper4 = ({ formData, setFormData, error, setError, handleNext, handleBack }) => {
  ...
  console.log("Loaded Stepper4.jsx (Grocery Seller)");
  const IMAGE_SERVER_URL = `${import.meta.env.VITE_API_IMAGE_URL}/grocery-seller-images`;

  console.log("Stepper4 props:", { formData, entityType: formData.entity_type, shopName: formData.shop_name });
  ...
  const uploadImages = async () => {
    if (!formData.shop_name || !formData.shop_name.trim()) {
      ...
      console.log("Validation failed: Invalid shop_name:", formData.shop_name);
      return false;
    }
    ...
    if (imageFiles.length === 0) {
      setError("At least one shop image is required.");
    ...
    try {
      ...
      let company_images = formData.company_images ? [...formData.company_images] : [];
      ...
      if (logoFile) {
        ...
        company_logo = logoResponse.data.logoUrl;
      }
      ...
      company_images = imagesResponse.data.files.map((file, index) => {
        if (!file.fileUrl) {
          throw new Error(`Missing fileUrl for file ${index}`);
        }
        return file.fileUrl;
      });
      console.log("Parsed company_images:", company_images);

      console.log("Updating formData:", { company_logo, company_images });
      setFormData((prev) => ({
        ...prev,
        company_logo,
        company_images,
  ...
  return (
    <div className="space-y-4">
      <div>
        <Label htmlFor="company_logo">Shop Logo * (Max 5MB)</Label>
        <Input
          id="company_logo"
          type="file"
        ...
        {formData.company_logo && !logoPreview && (
          <div className="mt-2">
            <img
      ...
      <div>
        <Label htmlFor="company_images">Shop Images * (Up to 5, Max 5MB each)</Label>
        <Input
          id="company_images"
          type="file"
          accept="image/jpeg,image/png,image/gif,image/webp"
        ...
        {formData.company_images && formData.company_images.length > 0 && imagePreviews.length === 0 && (
          <div className="mt-2 flex flex-wrap gap-2">
            {formData.company_images.map((url, index) => (
              <img
                key={index}
                src={url}
                alt={`Shop Image ${index + 1}`}
