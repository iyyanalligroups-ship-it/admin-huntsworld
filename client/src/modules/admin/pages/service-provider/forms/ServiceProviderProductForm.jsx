...
const ServiceProviderProductForm = ({ editingProduct }) => {
  ...
  // STEP 3: Set super_sub_category_id
  useEffect(() => {
  ...
  // STEP 4: Set deep_sub_category_id
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    const productPayload = {
      ...
      seller_id: selectedMerchant._id,
      sellerModel: "ServiceProvider",
      unitOfMeasurement: formData.unitOfMeasurement, // ← INCLUDED
    ...
    try {
      if (editingProduct) {
        ...
        if (res.success == true) {
          showToast(res.message || "Update Product Successfully",'success');
        } else {
        ...
        if (res.success) {
          showToast(res.message || "Product Add Successfully",'success');
        } else {
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                className={formData.askPrice ? "opacity-50 cursor-not-allowed hidden" : ""}
              />
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  if (value !== "other") {
                    ...
                    setFormData((prev) => ({ ...prev, unitOfMeasurement: customUnit }));
                  }
                }}
              >
                <SelectTrigger className="w-full">
              ...
              {selectedUnit === "other" && (
                <Input
                  type="text"
                  placeholder="Enter custom unit (e.g., Box, Set)"
                  value={customUnit}
                  onChange={(e) => {
                ...
                {formData.product_image.map((url, index) => (
                  <div key={index} className="relative w-fit">
                    ...
                    <Button
                      type="button"
                      size="icon"
                      onClick={() => handleImageDelete([url])} // ← FIXED: Delete single
