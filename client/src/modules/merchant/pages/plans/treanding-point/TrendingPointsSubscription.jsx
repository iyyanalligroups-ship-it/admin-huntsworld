...
const TrendingPointsSubscription = () => {
  ...
  const handlePurchase = async () => {
    try {
      ...
      console.log('Creating trending points order with:', { user_id: userId, points, amount });
  ...
  if (isTrendingPointsLoading || isFetching || isSubscriptionLoading || isConfigLoading || !user) {
    return (
      <div className="flex justify-center items-center p-6 bg-[#f0f4f6] rounded-xl">
        ...
        <span className="ml-3 text-gray-700 text-lg font-medium">Loading trending points subscription...</span>
      </div>
    );
  }
