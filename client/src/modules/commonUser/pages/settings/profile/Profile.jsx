...
const Profile = ({ userId }) => {
  ...
  const handleImageUpload = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        // ✅ Set to local state for immediate preview
        setProfileImage(uploadedUrl);
        setFormData((prev) => ({ ...prev, profile_pic: uploadedUrl }));

        // ✅ Call updateUser to persist in DB
        await updateUser({
          id: userId,
 ...
 const handleDeleteImage = async (profile_pic) => {
    console.log(profile_pic,"proileurl");
...
// Remove PropTypes for ParentComponent since userId is not a direct prop
