...
const ReviewProduct = () => {
  ...
  const { data: reviews, isLoading: reviewsLoading, error, refetch } = useGetReviewsByProductQuery(productId);
  ...
  if (isLoading || reviewsLoading) return <div className="p-4 text-center">Loading...</div>;
  if (error) return <div className="p-4 text-red-500 text-center">Error loading reviews: {error.message}</div>;

  const handleWriteReview = () => {
    // Safely access user._id
    const userId = user?.user?._id || user?._id;
    console.log("Write Review button clicked", { user, userId, token, isLoading });
    if (isLoading) {
    ...
    if (!user || !userId) {
      console.log("Redirecting to login due to missing user or userId", { user, token });
      toast({
      ...
      <Card className="shadow-lg">
        <CardHeader className="flex flex-row items-center justify-between bg-gray-50 p-4">
          ...
          <Button
            ...
            variant="default"
            className="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-md"
          >
      ...
      <div key={review._id} className="border-b pb-4">
        <div className="flex items-center gap-3">
          <Avatar className="w-10 h-10">
            <AvatarImage
              src={review.userId?.profile_pic || "/default-profile-pic.png"} // Fallback image
