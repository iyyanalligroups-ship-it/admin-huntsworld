...
const AdminTrustSealPlanManagement = ({ targetUser, hasSubscription, trustSealRequest, onRefresh, trustSealAmount, subscriptionId }) => {
  ...
  const handleAction = async () => {
    try {
      if (action === 'purchase') {
          ...
          handler: async (response) => {
            try {
              ...
              if (verifyRes.success) {
                showToast(`Payment for Trust Seal (₹${trustSealAmount}) completed. Awaiting verification.`, 'success');
        ...
        showToast(`Trust seal upgraded to ₹${upgradeAmount}`, 'success');
        setIsDialogOpen(false);
        setNotes('');
        ...
        showToast('Trust seal cancelled', 'error');
        setIsDialogOpen(false);
        setNotes('');
        onRefresh();
      }
    } catch (error) {
      showToast(`Failed to ${action} trust seal: ${error.message}`, 'error');
    } finally {
        ...
        {hasSubscription && (
          <div className="flex gap-2">
            {!trustSealRequest || trustSealRequest.status === 'rejected' ? (
              <Button
                className="bg-[#0c1f4d] cursor-pointer hover:bg-[#0c1f4dcc] text-white"
              ...
              <>
                {/* Pending – actions handled by admin review */}
              </>
            ) : (
              <>
                <Button
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              className={
                action === 'purchase' ? 'bg-[#0c1f4d] cursor-pointer hover:bg-[#0c1f4dcc]' :
                action === 'upgrade' ? 'bg-blue-600 cursor-pointer hover:bg-blue-700' :
                action === 'verified' ? 'bg-green-600 cursor-pointer hover:bg-green-700' :
