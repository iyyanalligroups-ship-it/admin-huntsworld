      ...
      <form onSubmit={handleSearch} className="mb-3 sm:mb-4">
        <div className="flex flex-col sm:flex-row gap-2">
          <Input
            ...
            className="flex-grow border-gray-300 focus:ring-2 focus:ring-[#0c1f4d] text-xs sm:text-sm"
          />
          <Button
            type="submit"
            className="bg-[#0c1f4d] cursor-pointer hover:bg-[#153171] text-white text-xs sm:text-sm px-3 py-1 sm:px-4 sm:py-2 min-w-[80px]"
          >
            Search
          </Button>
        </div>
      ...
      {searchResults && (
        <div className="mb-3 sm:mb-4 p-3 sm:p-4 border border-gray-200 bg-gray-50">
          ...
          <p className="mb-1 text-xs sm:text-sm text-gray-600">
            <span className="font-medium">Name:</span> {searchResults.name || "N/A"}
          </p>
          <p className="mb-1 text-xs sm:text-sm text-gray-600">
            <span className="font-medium">Email:</span> {searchResults.email || "N/A"}
          </p>
          <p className="mb-1 text-xs sm:text-sm text-gray-600">
            <span className="font-medium">Phone:</span> {searchResults.phone_number || "N/A"}
          </p>
          <p className="mb-2 sm:mb-3 text-xs sm:text-sm text-gray-600">
            <span className="font-medium">Role:</span> {searchResults.role?.role || "N/A"}
          </p>
          <Button
            onClick={handleNext}
...
const MerchantForm = ({ userId, onSubmit, onCancel, merchantRefresh }) => {
  ...
  return (
    <form
      ...
      className="p-3 sm:p-4 md:p-6 border border-gray-200 bg-white space-y-4"
    >
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className={`border-gray-300 focus:ring-2 focus:ring-[#0c1f4d] text-xs sm:text-sm ${errors.company_name ? "border-red-500" : ""
            }`}
      ...
      <div className="flex justify-end gap-2 sm:gap-3 pt-3 sm:pt-4">
        <Button
          ...
          variant="outline"
          onClick={onCancel}
          className="border-gray-300 cursor-pointer text-gray-700 hover:bg-gray-100 text-xs sm:text-sm px-3 py-1 sm:px-4 sm:py-2 min-w-[80px]"
        >
          Cancel
        </Button>
        ...
        <Button
          ...
          className={`flex items-center cursor-pointer gap-2 text-white text-xs sm:text-sm px-3 py-1 sm:px-4 sm:py-2 min-w-[100px] transition-all ${!isValid || isSubmitting
            ? "bg-gray-400 cursor-not-allowed"
            : "bg-[#0c1f4d] cursor-pointer hover:bg-[#153171]"
            }`}
        >
          {isSubmitting ? (
            <>
...
const MerchantList = () => {
  ...
  const fetchMerchants = async () => {
    ...
    try {
      ...
      // Sort: Unread first, then newest first (top-wise)
      const sortedMerchants = [...merchantsData].sort((a, b) => {
        if (a.mark_as_read !== b.mark_as_read) {
          return a.mark_as_read ? 1 : -1;
        }
      ...
      if (selectedMerchant) {
        const updatedMerchant = sortedMerchants.find(m => m._id === selectedMerchant._id);
        if (updatedMerchant) {
  ...
  const handleAddMerchant = async (formData) => {
    setMerchants((prev) => [...prev, formData]);
    setError(null);
  ...
  const handleSaveEdit = async (updatedMerchant, validationErrors = null) => {
    ...
    if (validationErrors && Object.keys(validationErrors).length > 0) {
      const errorMessages = Object.values(validationErrors)
        .filter((error) => error)
        .join("; ");
      setError(errorMessages || "Validation errors occurred. Please check the form.");
      return;
    }
    ...
    try {
      ...
      showToast(error.response?.data?.message || "Failed to update merchant.", "error");
    }
  };
  ...
  const handleToggleStatus = async (merchant) => {
    ...
    try {
      ...
      showToast(
        `Merchant ${newStatus ? "verified" : "unverified"} successfully`,
        "success"
      );
    } catch (error) {
  ...
  const handleToggle = async (userId) => {
    ...
    try {
      const currentMerchant = merchants.find(m => m.user_id?._id === userId);
      if (!currentMerchant?.user_id) return;

      const currentStatus = !!currentMerchant.user_id.isActive;
      ...
      setMerchants(prev =>
        prev.map(m =>
          m.user_id?._id === userId
            ? { ...m, user_id: { ...m.user_id, isActive: newStatus } }
            : m
        )
      );
      ...
      // ← Replace with your actual endpoint and payload format
      ...
      // Optional: sync with real response from server
      if (res.data?.isActive !== undefined) {
        setMerchants(prev =>
          prev.map(m =>
            m.user_id?._id === userId
              ? { ...m, user_id: { ...m.user_id, isActive: res.data.isActive } }
              : m
          )
        );
      }
      ...
      setMerchants(prev =>
        prev.map(m =>
          m.user_id?._id === userId
            ? { ...m, user_id: { ...m.user_id, isActive: !m.user_id.isActive } }
            : m
        )
      );
  ...
  const handleViewProducts = (merchant) => {
    ...
    navigate({
      pathname: "/sub-admin-dashboard/merchants/products",
  ...
  return (
    <div
      className={`p-3 sm:p-4 md:p-6 lg:p-8 ${isSidebarOpen ? "lg:ml-56" : "lg:ml-16"
        } bg-gray-100 min-h-screen transition-all duration-300`}
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
        <h1 className="text-md border-1 border-[#0c1f4d] text-[#153171] bg-gray-100 p-2 rounded-r-2xl font-bold">Merchant List</h1>
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          <div className="flex gap-2 sm:gap-3">
            <Button
              ...
              className="flex items-center cursor-pointer bg-[#0c1f4d] hover:bg-[#153171] text-white text-xs sm:text-sm px-3 py-1 sm:px-4 sm:py-2 min-w-[100px]"
            >
            ...
            >
              <div className="flex justify-between items-center">
                <div className="text-sm font-medium text-gray-900 flex items-center gap-2">
                  {indexOfFirstMerchant + index + 1}. <Truncate text={merchant.company_name || "-"} />
  ...
  disabled={loading && togglingId === merchant?.user_id?._id}
  className="flex items-center gap-2 cursor-pointer text-sm"
>
  {loading && togglingId === merchant?.user_id?._id ? (
    <>
    ...
    <>
      ...
      <span>Activate Account</span>
                    ...
                    {merchant.mark_as_read !== true && (
                      <DropdownMenuItem
                        onClick={() => handleMarkAsRead(merchant._id)}
                        className="text-xs sm:text-sm text-blue-600 hover:bg-blue-50 font-medium"
                      >
                        Mark as Read
                      </DropdownMenuItem>
                    )}
              ...
              <div className="mt-2 text-xs text-gray-600">
                ...
                <p><span className="font-medium">Type:</span> {merchant.company_type?.name || merchant.company_type || "-"}</p>
                <p><span className="font-medium">Employees:</span> {merchant.number_of_employees || "-"}</p>

                <p className="mt-1">
                  <span className="font-medium">Account Status:</span>{" "}
                  <span className={`${merchant?.user_id?.isActive ? "text-green-600" : "text-red-600"} font-medium`}>
                    {merchant?.user_id?.isActive ? "Active" : "Inactive"}
                  </span>
                </p>
            ...
            {currentMerchants.length > 0 ? (
              currentMerchants.map((merchant, index) => (
                <TableRow key={merchant._id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-xs sm:text-sm text-gray-600">
                    <span>{merchant.company_email || "-"} </span>
                  </TableCell>
                  <TableCell className="text-xs sm:text-sm text-gray-600">
                    {merchant.company_phone_number || "-"}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className={`w-2.5 h-2.5 rounded-full ${merchant?.user_id?.isActive ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className={`text-xs sm:text-sm font-medium ${merchant?.user_id?.isActive ? 'text-green-700' : 'text-red-700'}`}>
                        {merchant?.user_id?.isActive ? "Active" : "Inactive"}
                      </span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="bg-white shadow-lg">
                        ...
                        <DropdownMenuItem
                          onClick={() => handleViewProducts(merchant)}
                          className="text-xs sm:text-sm text-gray-700 hover:bg-gray-100 transition-colors"
                        >
                          View Products
                        </DropdownMenuItem>
                        <DropdownMenuItem
