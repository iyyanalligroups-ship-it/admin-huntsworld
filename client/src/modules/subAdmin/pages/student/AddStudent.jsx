...
function AddStudentModal({
  ...
  const [selectedUserId, setSelectedUserId] = useState(formData.user_id || null);
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 2 && isStep2Valid) {
      try {
        ...
        let addressId;
        if (isEditMode && formData.address_id) {
          const response = await axios.put(
            `${import.meta.env.VITE_API_URL}/address/update-address/${formData.user_id}/${formData.address_id}`,
            addressData
          );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? `Edit Student - Step ${currentStep} of ${totalSteps}` : `Add Student - Step ${currentStep} of ${totalSteps}`}
          </DialogTitle>
        </DialogHeader>
