...
const Truncate = ({ text }) => {
  ...
  return isLong ? (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <span className="inline-block max-w-[15ch] truncate cursor-default">
            {truncated}
          </span>
        </TooltipTrigger>
        <TooltipContent
    ...
    <span className="inline-block max-w-[15ch] truncate">{formattedText}</span>
  );
};
...
function AllStudents() {
  ...
  const {
    data: currentUser,
  } = useGetUserByIdQuery(user_id, { skip: !user_id });

  const currentPagePath = "students";
  ...
  const fetchStudents = async () => {
    try {
      ...
      const sorted = studentsWithDetails.sort((a, b) => {
        if (a.markAsRead !== b.markAsRead) return a.markAsRead ? 1 : -1;
      ...
      if (selectedStudent) {
        const updated = sorted.find(s => s._id === selectedStudent._id);
        if (updated) {
  ...
  const handleToggle = async (userId) => {
    ...
    try {
      ...
      setStudents(prev => prev.map(s => s.user_id === userId ? { ...s, isActive: newActiveState } : s));
  ...
  const handleToggleStatus = async (student) => {
    try {
      ...
      if (response.data.success) {
        showToast(
          response.data.verified ? `${student.name || "Student"} marked as Verified ✅` : `${student.name || "Student"} marked as Unverified ❌`,
          response.data.verified ? "success" : "warning"
        );
        setFilteredStudents(prev => prev.map(s => s._id === student._id ? { ...s, verified: response.data.verified } : s));
      }
    } catch (error) {
  ...
  const handleViewDetails = async (studentId) => {
    try {
      ...
      if (student.markAsRead !== true) handleMarkAsRead(student._id);
    } catch (error) {
  ...
  const handleEdit = async (studentId) => {
    try {
      ...
      setFormData({
        ...
        college_start_month_year: student.college_start_month_year ? new Date(student.college_start_month_year).toISOString().split("T")[0] : "",
        college_end_month_year: student.college_end_month_year ? new Date(student.college_end_month_year).toISOString().split("T")[0] : "",
        id_card: student.id_card || "",
      });
      setSelectedFile(null);
      setIdCardImage(student.id_card || null);
    } catch (error) {
      showToast("Failed to fetch student for edit", "error");
    }
  };
  ...
  const handleIdCardUpload = async () => {
    if (!selectedFile) return showToast("Select an image first", "error");
    const trimmedCollegeName = formData.college_name.trim();
    if (!trimmedCollegeName) return showToast("College name required", "error");
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_API_IMAGE_URL}/student-images/id-card/update/${encodeURIComponent(trimmedCollegeName)}`,
        formDataUpload,
  ...
  const handleIdCardDelete = async () => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_IMAGE_URL}/student-images/id-card/delete/${encodeURIComponent(trimmedCollegeName)}`
      );
  ...
  const confirmDelete = (studentId, userId, addressId, collegeName, student) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/students/delete-students-by-id/${selectedStudentId}`);
      if (selectedAddressId) {
        await axios.delete(`${import.meta.env.VITE_API_URL}/address/delete-address-addressId-userId/${selectedUserId}/${selectedAddressId}`);
      }
      if (selectedCollegeName) {
        await axios.delete(`${import.meta.env.VITE_API_IMAGE_URL}/student-images/id-card/delete/${encodeURIComponent(selectedCollegeName)}`);
      }
      ...
      <div className="min-h-screen sm:p-6 lg:p-8 max-w-[95vw] sm:max-w-7xl mx-auto">
        ...
        <div className="xl:col-span-1 mt-4">
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-[#0c1f4d] tracking-tight">Student Protocol</h2>
            <p className="text-sm text-slate-500 font-medium">Administrative guidelines for student management.</p>
          </div>

          <div className="flex flex-col md:flex-row gap-2 space-y-4 my-4 md:space-y-0">
            <Card className="border-l-4 border-l-indigo-600 shadow-sm bg-white">
              <CardHeader className="p-4 pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800">
                  <School size={16} className="text-indigo-600" /> 1. Institutional KYC
                </CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0">
                <p className="text-xs text-slate-600">The Verified Switch confirms enrollment. Validate via College Email or ID card.</p>
              </CardContent>
            </Card>
            ...
            <Card className="border-l-4 border-l-emerald-600 shadow-sm bg-white">
              <CardHeader className="p-4 pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800">
                  <UserCheck size={16} className="text-emerald-600" /> 2. Access Governance
                </CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0 text-xs text-slate-600">
                <li className="list-none"><strong>Active:</strong> Platform access enabled.</li>
                <li className="list-none"><strong>Inactive:</strong> Graduated or suspended.</li>
              </CardContent>
            </Card>
            ...
            <Card className="border-l-4 border-l-amber-500 shadow-sm bg-white">
              <CardHeader className="p-4 pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800">
                  <FileText size={16} className="text-amber-600" /> 3. Data Management
                </CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0 text-xs text-slate-600">
                <p>Ensure College Email is unique. Delete is permanent.</p>
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 pt-4 mb-6">
          <Input
            ...
            className="w-full sm:w-1/2 md:w-1/3 rounded-md border-gray-300 focus:ring-2 focus:ring-[#0c1f4d] text-sm"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
            <Button onClick={() => setIsAddModalOpen(true)} className="bg-[#0c1f4d] hover:bg-[#153171] text-white">
              <PlusCircle className="h-4 w-4 mr-2" /> Add Student
            </Button>
            <Button onClick={fetchStudents} className="bg-[#0c1f4d] hover:bg-[#153171] text-white">
        ...
        <div className="md:hidden space-y-4">
          {currentItems.map((student, index) => (
            <Card key={student._id} className="border rounded-lg shadow-sm hover:bg-gray-50 transition-colors">
              <CardContent className="p-4">
                <div className="flex justify-between items-center mb-3">
                  <p className="text-sm font-medium text-gray-900 truncate flex items-center gap-2">
                    {student.name || "N/A"}
                    {student.markAsRead !== true && <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full animate-pulse">New</span>}
                  </p>
                  <DropdownMenu>
                    <DropdownMenuTrigger><MoreVertical className="h-5 w-5 text-gray-500" /></DropdownMenuTrigger>
                    <DropdownMenuContent className="bg-white z-50">
                      <DropdownMenuItem onClick={() => handleViewDetails(student._id)}>View Details</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleToggle(student.user_id)} disabled={loadingStudents[student.user_id]} className="flex items-center gap-2">
                        {student.isActive ? <><ToggleLeft className="text-red-600" /> Deactivate</> : <><ToggleRight className="text-green-600" /> Activate</>}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleEdit(student._id)} disabled={!canEdit}>Edit</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => confirmDelete(student._id, student.user_id, student.address_id, student.college_name, student)} disabled={!canDelete} className="text-red-600">Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <p className="flex items-center gap-2"><span className="font-medium">Active:</span> <div className={`w-2 h-2 rounded-full ${student.isActive ? 'bg-green-500' : 'bg-red-500'}`} /></p>
                  <p className="flex items-center gap-2"><span className="font-medium">Verified:</span> <Switch checked={student.verified} onCheckedChange={() => handleToggleStatus(student)} /></p>
                </div>
              </CardContent>
            </Card>
          ))}
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-[#0c1f4d] border-b hover:bg-[#153171] border-gray-300 text-white">
                ...
                <TableHead className="text-white">Verified Status</TableHead>
                <TableHead className="text-white text-right pr-6">Action</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {currentItems.map((student, index) => (
                <TableRow key={student._id} className="hover:bg-gray-50">
                  <TableCell className="text-sm text-gray-600">{indexOfFirstItem + index + 1}</TableCell>
                  <TableCell className="text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      <Truncate text={student.name || "N/A"} />
                      {student.markAsRead !== true && <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full animate-pulse">New</span>}
                    </div>
                  </TableCell>
                  <TableCell className="text-sm text-gray-600">{student.college_email || "N/A"}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                  ...
                  <TableCell className="text-right pr-6">
                    <DropdownMenu>
                      <DropdownMenuTrigger><MoreVertical className="h-5 w-5 text-gray-500" /></DropdownMenuTrigger>
                      <DropdownMenuContent className="bg-white z-50">
                        <DropdownMenuItem onClick={() => handleViewDetails(student._id)} className="cursor-pointer">View Details</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleToggle(student.user_id)} disabled={loadingStudents[student.user_id]} className="flex items-center gap-2 cursor-pointer">
                          {student.isActive ? <><ToggleLeft className="w-4 h-4 text-red-500" /> Deactivate Account</> : <><ToggleRight className="w-4 h-4 text-green-500" /> Activate Account</>}
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleEdit(student._id)} disabled={!canEdit} className="cursor-pointer">Edit</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => confirmDelete(student._id, student.user_id, student.address_id, student.college_name, student)} disabled={!canDelete} className="text-red-600 cursor-pointer">Delete</DropdownMenuItem>
                        {student.markAsRead !== true && <DropdownMenuItem onClick={() => handleMarkAsRead(student._id)} className="text-blue-600 cursor-pointer">Mark as Read</DropdownMenuItem>}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                className="h-12 border-gray-200 focus:ring-[#0c1f4d] focus:border-[#0c1f4d]"
                              />
                            </div>
                            <div className="space-y-2">
        ...
        <DeleteDialog
          ...
          description="This will permanently delete the student identity, associated addresses, and verification documents. This action is irreversible."
        />
      </div>
    </div>
  );
