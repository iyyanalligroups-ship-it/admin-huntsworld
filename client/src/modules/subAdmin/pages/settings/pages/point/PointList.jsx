...
const PointList = ({ onEdit }) => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (response.success) {
        toast.success(response.message || "Point Deleted Successfully");
      } else {
        toast.error(response.message || "Failed to Delete Point");
      }
    } catch (error) {
        ...
        points?.data?.map((point) => (
          <div key={point._id} className="flex items-center justify-between p-4 border rounded-md">
            <div>
              ...
              <Badge className="bg-[#0c1f4d]">₹ {point.point_amount}</Badge>
              <p className="text-sm text-muted-foreground">
                Duration: {point.time_duration} {point.time_unit}
              </p>
            </div>
            <div className="flex gap-2">
